...
service.clonar = async (conn, id_padre, id_material_solicitud) => {
    try {
        const queryResponse = await conn.query(
            "INSERT INTO dino.tmodelo_pronostico_material_solicitud( \
            id_material_solicitud, id_modelo_pronostico, id_modelo_pronostico_borrador, modelo_pronostico_borrador, error) \
            SELECT $2, id_modelo_pronostico, id_modelo_pronostico_borrador, modelo_pronostico_borrador, error \
	        FROM dino.tmodelo_pronostico_material_solicitud \
            WHERE id_material_solicitud = $1;", [id_padre, id_material_solicitud]);

        return queryResponse.rows;
    } catch (error) {
        error.stack = "\nError en modeloPronosticoMaterialService.clonar, " + error.stack;
        throw error;
    }
};
