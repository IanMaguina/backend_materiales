...
service.clonar = async (conn, id_padre, id_material_solicitud) => {
    try {
        const queryResponse = await conn.query(
            "INSERT INTO dino.tgrupo_planif_necesidades_material_solicitud( \
            id_material_solicitud, id_grupo_planif_necesidades_borrador, grupo_planif_necesidades_borrador, grupo_planif_necesidades, error) \
            SELECT $2, id_grupo_planif_necesidades_borrador, grupo_planif_necesidades_borrador, grupo_planif_necesidades, error \
	        FROM dino.tgrupo_planif_necesidades_material_solicitud \
            WHERE id_material_solicitud = $1;", [id_padre, id_material_solicitud]);

        return queryResponse.rows;
    } catch (error) {
        error.stack = "\nError en grupoPlanifNecesidadesMaterialService.clonar, " + error.stack;
        throw error;
    }
};
