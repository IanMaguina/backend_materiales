...
service.obtenerRolEstaAqui = async (conn, id_solicitud) => {
    try {
        const queryResponse = await conn.query(
            "SELECT id_rol \
            FROM dino.taprobador_solicitud \
            WHERE id_solicitud = $1 AND tipo = 'F' AND esta_aqui = true", [id_solicitud]);
...
service.obtenerDestinatariosPorRol = async (conn, id_solicitud, id_rol) => {
    try {
        const queryResponse = await conn.query(
            "SELECT DISTINCT correo_usuario_real \
            FROM dino.taprobador_solicitud \
            WHERE id_solicitud = $1 AND tipo = 'F' AND id_rol_real = $2", [id_solicitud, id_rol]);
...
service.listarEmailsInternosExternos = async (conn, id_solicitud) => {
    try {
        const queryResponse = await conn.query(
            "SELECT distinct correo_usuario correo \
            FROM dino.taprobador_solicitud \
            WHERE id_solicitud = $1 AND tipo = 'F' \
            UNION \
            SELECT distinct ec.correo \
        ...
        error.stack = "\nError en aprobadorSolicitudService.listarEmailsInternosExternos, " + error.stack
        throw error;
    }
}
