...
service.clonar = async (conn, id_padre, id_material_solicitud) => {
    try {
        const queryResponse = await conn.query(
            "INSERT INTO dino.tarea_planificacion_material_solicitud( \
            id_material_solicitud, area_planificacion_borrador, area_planificacion, error) \
            SELECT $2, area_planificacion_borrador, area_planificacion, error \
	        FROM dino.tarea_planificacion_material_solicitud \
            WHERE id_material_solicitud = $1;", [id_padre, id_material_solicitud]);

        return queryResponse.rows;
    } catch (error) {
        error.stack = "\nError en areaPlanificacionService.clonar, " + error.stack;
        throw error;
    }
};
