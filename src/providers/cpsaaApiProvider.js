...
service.registrarMaterial = async (solicitud, list_regla_vista) => {
    ...
    console.log('******************************');
    const data = await fetchWithRetry(config.cpsaaSapApi.hostRegistrar, request);
    console.log('******************************');
...
service.consulta_material_nombre = async (denominaciones) => {
    console.log('cpsaaApiProvider.consulta_material_nombre');
    ...
    console.log('\n******************************');
    console.log('SAP [consulta_material_nombre] - Request...');
    console.log(request.body);

    console.log('\n******************************');
    console.log('SAP [consulta_material_nombre] - Enviando...');
    const data = await fetchWithRetry(config.cpsaaSapApi.hostConsultaMaterialNombre, request);

    console.log('\n******************************');
    console.log('SAP [consulta_material_nombre] - Response...');
    console.log(JSON.stringify(data));
...
service.consulta_material_codigo = async (material) => {
    console.log('cpsaaApiProvider.consulta_material_codigo');
    ...
    console.log('\n******************************');
    console.log('SAP [consulta_material_codigo] - Request...');
    console.log(request.body);

    console.log('\n******************************');
    console.log('SAP [consulta_material_codigo] - Enviando...');
    const data = await fetchWithRetry(config.cpsaaSapApi.hostConsultaMaterialCodigo, request);

    console.log('\n******************************');
    console.log('SAP [consulta_material_codigo] - Response...');
    console.log(JSON.stringify(data));
            ...
            if (data.resultado !== '') {
                ...
                for (let index = 0; index < data.resultado.TI_MATNR.length; index++) {
                    ...
                    result.material = {
                        ...
                        LVORM_A: element.LVORM_A,
                        LVORM_C: element.LVORM_C,
                        LVORM_G: element.LVORM_G,
                        LVORM_OV: element.LVORM_OV,
                        campos: mapping_material_response(element, data.resultado.TI_CLASES, data.resultado.TI_MDMA)
                    };
...
service.consulta_material_codigo_masivo = async (list_material) => {
    console.log('cpsaaApiProvider.consulta_material_codigo_masivo');
    ...
    console.log('\n******************************');
    console.log('SAP [consulta_material_codigo_masivo] - Request...');
    console.log(request.body);

    console.log('\n******************************');
    console.log('SAP [consulta_material_codigo_masivo] - Enviando...');
    const data = await fetchWithRetry(config.cpsaaSapApi.hostConsultaMaterialCodigo, request);

    console.log('\n******************************');
    console.log('SAP [consulta_material_codigo_masivo] - Response...');
    console.log(JSON.stringify(data));
...
function obtenerRegistroMaterialRequest(solicitud, list_regla_vista) {
    ...
    solicitud.Materiales.forEach(element => {
        ...
        var ti_solic = {
            ...
            N_CEN: obtener_flag_N_CEN(solicitud, element),
            N_ALM: obtener_flag_N_ALM(solicitud, element),
            N_OVEN: obtener_flag_N_OVEN(solicitud, element),
            ...
            //DEZEIT : Repetido en documentación            
        ...
        if (list_regla_vista && (solicitud.tip_pm == 'C' || solicitud.tip_pm == 'A')) {
...
function obtener_flag_N_CEN(solicitud, element) {
    if (solicitud.tip_pm == "C") {
        if (element.centro_codigo_sap && element.centro_codigo_sap != "") {
            return "X";
        }
    }
    else if (solicitud.tip_pm == "A") {
        if (element.centro_codigo_sap && element.centro_codigo_sap != "") {
            return "X";
        }
    }
    else if (solicitud.tip_pm == "M") {
        return "";
    }
    else if (solicitud.tip_pm == "B") {
        if (element.centro_codigo_sap && element.centro_codigo_sap != "") {
            if (!element.almacen_codigo_sap || element.almacen_codigo_sap == "") {
                return "X";
            }
        }
    }
...
function obtener_flag_N_ALM(solicitud, element) {
    if (solicitud.tip_pm == "C") {
        if (element.almacen_codigo_sap && element.almacen_codigo_sap != "") {
            return "X";
        }
...
function obtener_flag_N_OVEN(solicitud, element) {
    if (solicitud.tip_pm == "C") {
        if (element.organizacion_ventas && element.organizacion_ventas != "") {
            return "X";
        }
    }
    else if (solicitud.tip_pm == "A") {
...
function obtener_flag_V_PRON(material, list_regla_vista) {
    ...
    if (reglas) {
        for (let index = 0; index < reglas.length; index++) {
            ...
            else if (regla.codigo_interno == enums.codigo_interno.anular_automaticamente) {
                if (material.AUTRU != "") {
                    return "X";
                }
            }

            else if (regla.codigo_interno == enums.codigo_interno.optimizacion_parametro) {
                if (material.KZPAR != "") {
                    return "X";
                }
            }
...
function mapping_material_response(element, clasificaciones, areas_planificacion) {
    const lista = [];

    if (clasificaciones && clasificaciones.length > 0) {
        let campo = { codigo_interno: enums.codigo_interno.clasificacion_tab, valores: [] };

        clasificaciones.forEach(clasificacion => {
            if (clasificacion.MATNR === element.MATNR) {
                campo.valores.push({ valor: clasificacion.CLASSNUM });
            }
        });
    ...
    if (areas_planificacion && areas_planificacion.length > 0) {
        let campo = { codigo_interno: enums.codigo_interno.area_planificacion_tab, valores: [] };

        areas_planificacion.forEach(area_planificacion => {
            if (area_planificacion.MATNR === element.MATNR) {
                campo.valores.push({ valor: area_planificacion.BERID });
            }
        });
    ...
    if (!valor_invalido(element.STAWN)) {
        lista.push({ codigo_interno: enums.codigo_interno.partida_arancelaria, valor: element.STAWN });
    }
    ...
    if (!valor_invalido(element.KTGRM)) {
        lista.push({ codigo_interno: enums.codigo_interno.grupo_imputacion_material, valor: element.KTGRM });
    }

    if (!valor_invalido(element.PRODH)) {
        lista.push({ codigo_interno: enums.codigo_interno.jerarquia_producto, valor: element.PRODH });
    }

    if (!valor_invalido(element.MVGR1)) {
        lista.push({ codigo_interno: enums.codigo_interno.grupo_material1, valor: element.MVGR1 });
    }

    if (!valor_invalido(element.MVGR2)) {
        lista.push({ codigo_interno: enums.codigo_interno.grupo_material2, valor: element.MVGR2 });
    }
    ...
    if (!valor_invalido(element.PRMOD)) {
        lista.push({ codigo_interno: enums.codigo_interno.modelo_pronostico, valor: element.PRMOD });
    }
    ...
    if (!valor_invalido(element.KZPAR)) {
        lista.push({ codigo_interno: enums.codigo_interno.optimizacion_parametro, valor: element.KZPAR });
    }
    ...
    if (!valor_invalido(element.KAUTB)) {
        lista.push({ codigo_interno: enums.codigo_interno.ind_ped_automa, valor: element.KAUTB });
    }
