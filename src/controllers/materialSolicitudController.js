...
controller.external.agregarMaterial = async (req, res) => {
    ...
    try {
        ...
        if (!id_solicitud || id_solicitud == constantes.emptyString) {
            res.status(200).json({ resultado: 0, mensaje: "El campo id_solicitud es obligatorio" });
            return;
        }
        //#endregion
        ...
        if (!material) {
            res.status(200).json({ resultado: 0, mensaje: "El campo material es obligatorio" });
            return;
        }
        //#endregion

        //#region 3. Validar campos en request
        if (!material.campos || material.campos.length === 0) {
            res.status(200).json({ resultado: 0, mensaje: "El campo material debe contener campos" });
            return;
        }
        //#endregion 

        //#region 4. Validar existe id_solicitud
        const solicitud = await solicitudService.obtenerParaValidar(postgresConn, id_solicitud);
        if (!solicitud) {
            res.status(200).json({ resultado: 0, mensaje: "No existe el id_solicitud {" + id_solicitud + "}" });
            return;
        }
        //#endregion

        //#region 5. Validar denominacion
        const request_denominacion = obtenerCampoValor(enums.codigo_interno.denominacion, material.campos);
        if (!request_denominacion) {
            res.status(200).json({ resultado: 0, mensaje: "El campo denominación es obligatorio." });
            return;
        }
        //#endregion

        //#region 6. Validar centro
        const request_centro = obtenerCampoValor(enums.codigo_interno.centro_codigo_sap, material.campos);
        if (false) {
            if (!request_centro) {
                res.status(200).json({ resultado: 0, mensaje: "El campo centro_codigo_sap es obligatorio." });
                return;
            }
        }
        //#endregion

        //#region 7. Validar almacen
        const request_almacen = obtenerCampoValor(enums.codigo_interno.almacen_codigo_sap, material.campos);
        if (false) {

            if (!request_almacen) {
                res.status(200).json({ resultado: 0, mensaje: "El campo almacen_codigo_sap es obligatorio." });
                return;
            }
        }
        //#endregion

        const db_materiales = await materialSolicitudService.listarMaterialesParaValidar(postgresConn, id_solicitud);

        //#region 8. Validar Ampliación
        const request_ampliacion = obtenerCampoValor(enums.codigo_interno.ampliacion, material.campos);
        let padre = null;
        if (request_ampliacion === "X") {
            //#region 8.1 Validar padre
            for (let index = 0; index < db_materiales.length; index++) {
                const db = db_materiales[index];

                if (!db.ampliacion) {
                    if (request_denominacion === db.denominacion) {
                        padre = db;
                        break;
                    }
                }
            ...
            if (padre) {
                const request_clave = request_centro + '_' + request_almacen;
                const padre_clave = padre.centro_codigo_sap + '_' + padre.almacen_codigo_sap;

                if (request_clave === padre_clave) {
                    res.status(200).json({ resultado: 0, mensaje: "La ampliación debe tener diferente centro/almacen que el material a ampliar" });
                    return;
                }
            } else {
                res.status(200).json({ resultado: 0, mensaje: "No existe un material para ampliar con la denominación " + [request_denominacion] });
                return;
            }
            //#endregion
            //#region 8.2 Validar ya existe
            for (let index = 0; index < db_materiales.length; index++) {
                const db = db_materiales[index];
                ...
                if (db.ampliacion === "X") {
                    ...
                    if (request_clave === db_clave) {
                        res.status(200).json({ resultado: 0, mensaje: "Ya existe una ampliación en esta solicitud para el centro/almacen seleccionado." });
                        return;
                    }
                }
            }
        ...
        //#region 9. Validar existe en db
        if (request_ampliacion !== "X") {
            const denominacionesEnDb = await materialSolicitudService.listarPorDenominacion(postgresConn, request_denominacion);
            if (denominacionesEnDb && denominacionesEnDb.length > 0) {
                let mensaje = "Ya existe un material en trámite con la denominación [" + request_denominacion + "] en otra solicitud.";

                for (let index = 0; index < denominacionesEnDb.length; index++) {
                    const element = denominacionesEnDb[index];

                    if (element.id === id_solicitud) {
                        mensaje = "Ya existe un material en trámite con la denominación [" + request_denominacion + "] en esta solicitud.";
                        break;
                    }
                }
                ...
                res.status(200).json({
                    resultado: 0,
                    mensaje: mensaje,
                    lista: { id_solicitud: id_solicitud }
                });
                return;
            }
        }
        ...
        //#region 10. Validar existe en SAP
        if (request_ampliacion !== "X") {
            const denominacionesEnSAP = await cpsaaApiProvider.consulta_material_nombre([request_denominacion]);
            var existeEnSAP = await existeDenominacionEnSAP(denominacionesEnSAP, request_denominacion);
            if (existeEnSAP) {
                res.status(200).json({ resultado: 0, mensaje: "Ya existe una solicitud en SAP con la denominación [" + request_denominacion + "]." });
                return;
            }
        }
        //#endregion

        let id_material_solicitud = 0;
        await client.query("BEGIN");

        if (padre) {
            id_material_solicitud = await clonarPadre(client, padre.id_material_solicitud, request_centro, request_almacen);
        } else {
            const materiales = [];
        ...
        res.status(200).json({ resultado: 1, mensaje: "", lista: [obtenerMaterial] });
...
controller.external.agregarMaterial_old = async (req, res) => {
    const client = await postgresConn.getClient();
...
controller.external.agregarMateriales = async (req, res) => {
    const client = await postgresConn.getClient();
    const response = { resultado: 0, mensaje: "Error inesperado al agregar Materiales." };
    ...
    try {
        ...
        //#region 1. Validar id_solicitud en request
        if (!id_solicitud || id_solicitud == constantes.emptyString) {
            res.status(200).json({ resultado: 0, mensaje: "El campo id_solicitud es obligatorio", carga_masiva: false });
            return;
        }
        //#endregion

        //#region 2. Validar id_usuario en request
        if (!id_usuario || id_usuario == constantes.emptyString) {
            res.status(200).json({ resultado: 0, mensaje: "El campo id_usuario es obligatorio", carga_masiva: false });
            return;
        }
        //#endregion

        //#region 3. Validar usuario en request
        const aprobador_solicitud = aprobadorSolicitudService.listarParaValidar(postgresConn, id_solicitud, id_usuario);
        if (!aprobador_solicitud || aprobador_solicitud.length === 0) {
            res.status(200).json({ resultado: 0, mensaje: "Usuario inválido para registrar materiales", carga_masiva: false });
            return;
        }
        //#endregion

        //#region 4. Validar material en request
        if (!materiales || materiales.length === 0) {
            res.status(200).json({ resultado: 0, mensaje: "El campo materiales es obligatorio", carga_masiva: false });
            return;
        }
        //#endregion

        //#region 5. Validar existe id_solicitud
        const solicitud = await solicitudService.obtenerParaValidar(postgresConn, id_solicitud);
        if (!solicitud) {
            res.status(200).json({ resultado: 0, mensaje: "No existe el id_solicitud {" + id_solicitud + "}", carga_masiva: false });
            return;
        }
        //#endregion

        //#region 6. Validar existe id_material_solicitud
        const request_id_materiales = [];
        for (let index = 0; index < materiales.length; index++) {
            const material = materiales[index];

            if (material.id_material_solicitud) {
                request_id_materiales.push(material.id_material_solicitud);
            }
        }

        if (request_id_materiales.length > 0) {
            const db_id_materiales = await materialSolicitudService.listarIdMaterialesParaValidar(postgresConn, id_solicitud, request_id_materiales);

            for (let index = 0; index < materiales.length; index++) {
                const material = materiales[index];

                if (material.id_material_solicitud) {
                    var exists = db_id_materiales.some(function (item) { return item.id == material.id_material_solicitud });
        ...
        //#region 7. Validar denominacion
        for (let index = 0; index < materiales.length; index++) {
            const material = materiales[index];

            const request_denominacion = obtenerCampoValor(enums.codigo_interno.denominacion, material.campos);
            if (!request_denominacion) {
                errores.push("- Fila [" + (index + 1) + "] - El campo denominación es obligatorio. \n");
            }
        }
        ...
        //#region 8. Validar centro_codigo_sap
        if (false) {
            for (let index = 0; index < materiales.length; index++) {
                const material = materiales[index];
                ...
                const request_centro = obtenerCampoValor(enums.codigo_interno.centro_codigo_sap, material.campos);
                if (!request_centro) {
                    errores.push("- Fila [" + (index + 1) + "] - El campo centro_codigo_sap es obligatorio. \n");
                }
            }
        ...
        //#region 9. Validar almacen_codigo_sap
        if (false) {
            for (let index = 0; index < materiales.length; index++) {
                const material = materiales[index];
                ...
                const request_almacen = obtenerCampoValor(enums.codigo_interno.almacen_codigo_sap, material.campos);
                if (!request_almacen) {
                    errores.push("- Fila [" + (index + 1) + "] - El campo almacen_codigo_sap es obligatorio. \n");
                }
            }
        ...
        for (let index = 0; index < materiales.length; index++) {
            ...
            if (material.id_material_solicitud) {
                const db_material = db_materiales.filter(function (item) {
                    return item.id_material_solicitud == material.id_material_solicitud;
                })[0];

                if (request_ampliacion !== db_material.ampliacion) {
                    errores.push("- Fila [" + (index + 1) + "] - No es posible cambiar el flag de ampliacion. \n");
                    continue;
                }
            } else {
                const request_denominacion = obtenerCampoValor(enums.codigo_interno.denominacion, material.campos);
                ...
                if (request_ampliacion === "X") {
                    //#region 8.1 Validar padre
                    for (let index1 = 0; index1 < db_materiales.length; index1++) {
                        const db = db_materiales[index1];
                    ...
                    if (!material.padre) {
                        for (let index1 = 0; index1 < material.index; index1++) {
                            const excel = materiales[index1];
                            const excel_denominacion = obtenerCampoValor(enums.codigo_interno.denominacion, excel.campos);
                            const excel_ampliacion = obtenerCampoValor(enums.codigo_interno.ampliacion, excel.campos);

                            if (!excel_ampliacion) {
                                if (request_denominacion === excel_denominacion) {
                                    const excel_centro = obtenerCampoValor(enums.codigo_interno.centro_codigo_sap, excel.campos);
                                    const excel_almacen = obtenerCampoValor(enums.codigo_interno.almacen_codigo_sap, excel.campos);
                                    material.padre = { centro_codigo_sap: excel_centro, almacen_codigo_sap: excel_almacen, index: index1 };
                                    break;
                                }
                            }
                    ...
                    if (material.padre) {
                        const request_clave = request_centro + '_' + request_almacen;
                        const padre_clave = material.padre.centro_codigo_sap + '_' + material.padre.almacen_codigo_sap;

                        if (request_clave === padre_clave) {
                            errores.push("- Fila [" + (index + 1) + "] - La ampliación debe tener diferente centro/almacen que el material a ampliar. \n");
                            continue;
                        }
                    } else {
                        errores.push("- Fila [" + (index + 1) + "] - No existe un material para ampliar con la denominación " + request_denominacion + ". \n");
                        continue;
                    }
                    //#endregion
                    ...
                    for (let index1 = 0; index1 < db_materiales.length; index1++) {
                        ...
                        if (db.ampliacion === "X") {
                            ...
                            if (request_clave === db_clave) {
                                errores.push("- Fila [" + (index + 1) + "] - Ya existe una ampliación para el centro/almacen seleccionado. \n");
                                continue;
                            }
                        }
                    }
        ...
        //#region 11. Validar existe en db
        for (let index = 0; index < materiales.length; index++) {
            const material = materiales[index];
            ...
            if (material.id_material_solicitud) {
                ...
                if (request_ampliacion !== "X") {
                    const denominacionesEnDb = await materialSolicitudService.listarPorDenominacion(postgresConn, request_denominacion);
                    if (denominacionesEnDb && denominacionesEnDb.length > 0) {
                        errores.push("- Fila [" + (index + 1) + "] - Ya existe una solicitud en trámite con la denominación " + request_denominacion + ". \n");
                        continue;
                    }
                }
            }
        ...
        //#region 12. Validar existe en SAP
        const list_denominaciones = [];
        for (let index = 0; index < materiales.length; index++) {
            const material = materiales[index];

            if (!material.id_material_solicitud) {
                const request_denominacion = obtenerCampoValor(enums.codigo_interno.denominacion, material.campos);
                var exists = list_denominaciones.some(function (item) {
                    return item == request_denominacion;
                });

                if (!exists) {
                    list_denominaciones.push(request_denominacion);
                }
            }
        }

        if (list_denominaciones.length > 0) {
            const denominacionesEnSAP = await cpsaaApiProvider.consulta_material_nombre(list_denominaciones);
            ...
            for (let index = 0; index < materiales.length; index++) {
                ...
                if (material.id_material_solicitud) {
                    ...
                    if (request_ampliacion !== "X") {
                        ...
                        var existeEnSAP = await existeDenominacionEnSAP(denominacionesEnSAP, request_denominacion);
                        if (existeEnSAP) {
                            errores.push("- Fila [" + (index + 1) + "] - Ya existe una solicitud en SAP con la denominación " + request_denominacion + ". \n");
                            continue;
                        }
                    }
                }
        ...
        await asyncForEach(materialesValidados, async (materialValidado) => {
            ...
            let id_material_solicitud = materialValidado.id_material_solicitud;

            if (id_material_solicitud) {
                ...
                if (materialValidado.padre) {
                    const request_centro = obtenerCampoValor(enums.codigo_interno.centro_codigo_sap, materialValidado.campos);
                    const request_almacen = obtenerCampoValor(enums.codigo_interno.almacen_codigo_sap, materialValidado.campos);
                    const id_padre = materialesValidados[materialValidado.padre.index].id_material_solicitud;
                    await clonarPadre(client, id_padre, request_centro, request_almacen);
                } else {
                    const resultOriginal = await materialSolicitudService.crear(client, id_solicitud, materialValidado);
                    materialValidado.id_material_solicitud = resultOriginal[0].id;
                    await materialSolicitudService.crearBorrador(client, id_solicitud, materialValidado.id_material_solicitud, materialValidado);
                    await crearRegistrosEnTablas(client, materialValidado.id_material_solicitud, materialValidado);
                }
            }
        });
        ...
        res.status(200).json({ resultado: 1, mensaje: "", lista: obtenerMateriales, carga_masiva: true });
...
controller.external.actualizarMaterial = async (req, res) => {
    ...
    try {
        ...
        //#region 2. Validar id_material_solicitud en request
        if (!id_material_solicitud || id_material_solicitud == constantes.emptyString) {
            res.status(200).json({ resultado: 0, mensaje: "El campo id_material_solicitud es obligatorio" });
            return;
        }
        //#endregion
        ...
        //#region 6. Validar existe id_material_solicitud
        const db_material = await materialSolicitudService.obtenerParaValidar(postgresConn, id_material_solicitud);
        if (!db_material) {
            res.status(200).json({ resultado: 0, mensaje: "No existe el id_material_solicitud {" + id_material_solicitud + "}" });
            return;
        }
        //#endregion  
        ...
        let cambio_ampliacion = false;
        if (campo_ampliacion) {
            if (campo_ampliacion.valor != db_material.ampliacion) {
                cambio_ampliacion = true;
                let padre = null;

                if (campo_ampliacion.valor === "X") {
                    //#region 10.1 Validar padre
                    for (let index = 0; index < db_materiales.length; index++) {
                        const db = db_materiales[index];

                        if (db.id_material_solicitud != id_material_solicitud) {
                            if (!db.ampliacion) {
                                if (request_denominacion === db.denominacion) {
                                    padre = db;
                                    break;
                    ...
                    for (let index = 0; index < db_materiales.length; index++) {
                        ...
                        if (db.id_material_solicitud != id_material_solicitud) {
                            if (db.ampliacion === "X") {
                                ...
                                if (request_clave === db_clave) {
                                    res.status(200).json({ resultado: 0, mensaje: "Ya existe una ampliación para el centro/almacen seleccionado." });
                                    return;
                                }
                            }
                        }
        ...
        //#region 11. Validar existe en db
        if (cambio_ampliacion && campo_ampliacion.valor !== 'X') {
            if (request_denominacion !== db_material.denominacion) {
                const denominacionesEnDb = await materialSolicitudService.listarPorDenominacion(postgresConn, request_denominacion);
                if (denominacionesEnDb && denominacionesEnDb.length > 0) {
                    res.status(200).json({ resultado: 0, mensaje: "Ya existe una solicitud en trámite con la denominación [" + db_material.denominacion + "]." });
                    return;
                }
            } else {
                res.status(200).json({ resultado: 0, mensaje: "Ya existe una solicitud en trámite con la denominación [" + db_material.denominacion + "]." });
        ...
        if (cambio_ampliacion && campo_ampliacion.valor !== 'X') {
            if (request_denominacion !== db_material.denominacion) {
                ...
                var existeEnSAP = await existeDenominacionEnSAP(denominacionesEnSAP, request_denominacion);
                if (existeEnSAP) {
                    res.status(200).json({ resultado: 0, mensaje: "Ya existe una solicitud en SAP con la denominación [" + db_material.denominacion + "]." });
                    return;
                }
            }
        }
...
controller.external.agregarAmpliacion = async (req, res) => {
    const client = await postgresConn.getClient();
    ...
    try {
        ...
        const response_sap = await cpsaaApiProvider.consulta_material_codigo({
        ...
        const response_sap_ampliacion = await cpsaaApiProvider.consulta_material_codigo(request_sap_ampliacion);
        ...
        await client.query("ROLLBACK");
        winston.info("Error en materialSolicitudController.external.agregarAmpliacion. Details: ", error);
        res.status(500).send(error);
    } finally {
        client.release();
    }
...
controller.external.agregarAmpliaciones = async (req, res) => {
    ...
    try {
        ...
        if (list_material_codigo_sap.length > 0) {
            response_sap_masivo = await cpsaaApiProvider.consulta_material_codigo_masivo(list_material_codigo_sap);
            for (let index = 0; index < materiales.length; index++) {
                const material = materiales[index];
...
controller.external.agregarModificacion = async (req, res) => {
    const client = await postgresConn.getClient();
    ...
    try {
        ...
        const request_sap = {
            ...
            organizacion_ventas: material.organizacion_ventas,
            canal_distribucion: material.canal_distribucion,
            area_planificacion: material.area_planificacion
        }
        const response_sap = await cpsaaApiProvider.consulta_material_codigo(request_sap);
        ...
        const nuevo_material = { campos: response_sap.material.campos };
        material.campos.forEach(element => {
            if (element.valor || element.tipo_objeto=="checkbox" || (element.valores && element.valores[0] && element.valores[0].valor)) {
                nuevo_material.campos = removerCampo(element.codigo_interno, nuevo_material.campos);
                nuevo_material.campos.push(element);
            }
        });
        ...
        await client.query("ROLLBACK");
        winston.info("Error en materialSolicitudController.external.agregarModificacion. Details: ", error);
        res.status(500).send(error);
    } finally {
        client.release();
    }
...
controller.external.agregarModificaciones = async (req, res) => {
    ...
    try {
        ...
        if (list_material_sap.length > 0) {
            response_sap_masivo = await cpsaaApiProvider.consulta_material_codigo_masivo(list_material_sap);

            if (response_sap_masivo && response_sap_masivo.codigo === 1) {
                for (let index = 0; index < materiales.length; index++) {
                    const material = materiales[index];
                ...
                res.status(200).json({ resultado: 0, mensaje: "Error al intentar conectarse a SAP." });
                return;
            }
        }
...
controller.external.agregarBloqueo = async (req, res) => {
    const client = await postgresConn.getClient();
    ...
    try {
        ...
        const response_sap = await cpsaaApiProvider.consulta_material_codigo(request_sap);
        if (response_sap && response_sap.codigo === 1) {
            if (response_sap.material) {
                if (material.material_codigo_sap != response_sap.material.material_codigo_sap ||
                    (material.centro_codigo_sap ?? '') != response_sap.material.centro_codigo_sap ||
                    (material.almacen_codigo_sap ?? '') != response_sap.material.almacen_codigo_sap) {

                    res.status(200).json({ resultado: 0, mensaje: "No existe el material en SAP." });
                    return;
                } else {
                    if (response_sap.material.LVORM_A === 'X') {
                        res.status(200).json({ resultado: 0, mensaje: "Petición de borrado fijado a nivel de almacén." });
                        return;
                    }
                    else if (response_sap.material.LVORM_C === 'X') {
                        res.status(200).json({ resultado: 0, mensaje: "Como mínimo se han fijado dos peticiones de borrado." });
                        return;
                    }
                    else if (response_sap.material.LVORM_G === 'X') {
                        res.status(200).json({ resultado: 0, mensaje: "Petición de borrado fijada a nivel de mandante." });
                        return;
                    }
                }
            } else {
        ...
        await client.query("ROLLBACK");
        winston.info("Error en materialSolicitudController.external.agregarBloqueo. Details: ", error);
        res.status(500).send(error);
    } finally {
        client.release();
    }
            ...
            if (response_sap_masivo && response_sap_masivo.codigo === 1) {
                for (let index = 0; index < materiales.length; index++) {
                    ...
                    if (!material.id_material_solicitud) {
                        ...
                        if (response_sap && response_sap.length > 0) {
                            const sap = response_sap[0];

                            if (sap.LVORM_A === 'X') {
                                errores.push("- Fila [" + (index + 1) + "] - Petición de borrado fijado a nivel de almacén. \n");
                                continue;
                            }

                            else if (sap.LVORM_C === 'X') {
                                errores.push("- Fila [" + (index + 1) + "] - Como mínimo se han fijado dos peticiones de borrado. \n");
                                continue;
                            }

                            else if (sap.LVORM_G === 'X') {
                                errores.push("- Fila [" + (index + 1) + "] - Petición de borrado fijada a nivel de mandante. \n");
                                continue;
                            }
                        }
                        else {
                            let mensaje = "- Fila [" + (index + 1) + "] - El codigo_material_sap {" + material.material_codigo_sap + "}";

                            if (material.centro_codigo_sap) {
                                mensaje += " + centro_codigo_sap {" + material.centro_codigo_sap + "}";
                            }

                            if (material.almacen_codigo_sap) {
                                mensaje += " + almacen_codigo_sap {" + material.almacen_codigo_sap + "}";
                            }
...
controller.external.eliminarMaterial = async (req, res) => {
    ...
    try {
        ...
        //#region 2. Validar existe solicitud
        const solicitud = await solicitudService.obtenerParaValidar(postgresConn, id_solicitud);
        if (!solicitud) {
            res.status(200).json({ resultado: 0, mensaje: "No existe el id_solicitud {" + id_solicitud + "}" });
            return;
        ...
        if (material) {
            ...
            if (solicitud.id_tipo_solicitud === enums.tipo_solicitud.creacion && material.ampliacion !== 'X') {
            ...
            try {
                await notificacionController.internal.notificarEliminacionMaterial(id_solicitud, material);
            } catch (error) {
                winston.error("Error notificando la anulación,", error);
            }

            res.status(200).json({ resultado: 1, mensaje: "" });
        } else {
            res.status(200).send({ resultado: 0, mensaje: "No se encontró el Material para eliminar" });
        }
    } catch (error) {
        await client.query("ROLLBACK");
...
controller.external.existeDenominacion = async (req, res) => {
    try {
        ...
        const result = await cpsaaApiProvider.consulta_material_nombre([denominacion]);
...
controller.external.consultaCodigoMaterialSAP = async (req, res) => {
    try {
        ...
        const result = await cpsaaApiProvider.consulta_material_codigo(material);//consultarCodigoMaterial

        response.resultado = result.codigo;
        response.mensaje = result.mensaje;
        response.lista = result.material ? result.material.campos : [];
...
controller.external.consultaCodigoMaterialSAPAmpliacion = async (req, res) => {
    try {
        const response = {
            resultado: 0,
            mensaje: "Error inesperado al consultar el código del material."
        ...
        const result = await cpsaaApiProvider.consulta_material_codigo(material);//consultarCodigoMaterialAmpliacion
        ...
        winston.info("Error en materialSolicitudController.external.consultaCodigoMaterialSAPAmpliacion. Details: ", error);
        res.status(500).send(error);
    }
};
...
controller.external.consultarMaterialSAP = async (req, res) => {
    try {
        ...
        const result = await cpsaaApiProvider.consulta_material_codigo(material);//consultarMaterialSAP
...
controller.external.obtenerPadre = async (req, res) => {
    try {
        const response = {
            resultado: 0,
            mensaje: "Error inesperado al validar si existe la padre de ampliación en Base de Datos."
        ...
        const count = await materialSolicitudService.obtenerPadre(postgresConn, id_solicitud, denominacion);

        response.resultado = 1;
        response.mensaje = "";
        response.existe = (count ? true : false);
        response.lista = count;


        res.status(200).json(response);
    } catch (error) {
        winston.info("Error en materialSolicitudController.external.obtenerPadre. Details: ", error);
        res.status(500).send(error);
    }
};
//#endregion
...
async function enviarSAP(id_solicitud, tip_pm) {
    try {
        const sol = await solicitudService.obtenerParaValidar(postgresConn, id_solicitud);

        switch (sol.id_tipo_solicitud) {
            case 1: tip_pm = 'C'; break;
            case 2: tip_pm = 'A'; break;
            case 3: tip_pm = 'M'; break;
            case 4: tip_pm = 'B'; break;
            default: tip_pm = 'C'
        }
        ...
        const list_clase_inspeccion = await claseInspeccionMaterialService.listarParaEnviarSAP(postgresConn, id_solicitud);
        ...
        solicitud.ClaseInspeccion = list_clase_inspeccion;
...
async function validarMateriales(id_solicitud, materiales, tipo_carga, tipo_solicitud) {
    ...
    await asyncForEach(materiales, async (material) => {
        ...
        if (existeRegla(enums.codigo_interno.grupo_imputacion_material, reglas) && existeCampo(enums.codigo_interno.grupo_imputacion_material, material.campos)) {
            ...
            materialResult = await materialValidator.validar_codigo_sap(enums.codigo_interno.grupo_imputacion_material, material, materialResult, 2, 20, list_grupo_imputacion_material);
        }
        ...
        if (existeRegla(enums.codigo_interno.jerarquia_producto, reglas) && existeCampo(enums.codigo_interno.jerarquia_producto, material.campos)) {
            ...
            materialResult = await materialValidator.validar_codigo_sap(enums.codigo_interno.jerarquia_producto, material, materialResult, 18, 20, list_jerarquia_producto);
        }
        ...
        if (existeRegla(enums.codigo_interno.grupo_material1, reglas) && existeCampo(enums.codigo_interno.grupo_material1, material.campos)) {
            ...
            materialResult = await materialValidator.validar_codigo_sap(enums.codigo_interno.grupo_material1, material, materialResult, 3, 20, list_grupo_material1);
        }
        ...
        if (existeRegla(enums.codigo_interno.grupo_material2, reglas) && existeCampo(enums.codigo_interno.grupo_material2, material.campos)) {
            ...
            materialResult = await materialValidator.validar_codigo_sap(enums.codigo_interno.grupo_material2, material, materialResult, 3, 20, list_grupo_material2);
        }
        ...
        // [98]	anular_automaticamente
        if (existeRegla(enums.codigo_interno.anular_automaticamente, reglas) && existeCampo(enums.codigo_interno.anular_automaticamente, material.campos)) {
            materialResult = await materialValidator.validar_texto(enums.codigo_interno.anular_automaticamente, material, materialResult, 1, 20);
        }

        // [99]	optimizacion_parametro
        if (existeRegla(enums.codigo_interno.optimizacion_parametro, reglas) && existeCampo(enums.codigo_interno.optimizacion_parametro, material.campos)) {
            materialResult = await materialValidator.validar_texto(enums.codigo_interno.optimizacion_parametro, material, materialResult, 1, 20);
        }
        ...
        if (tipo_solicitud == enums.tipo_solicitud.creacion) {
            if (existeCampo(enums.codigo_interno.ampliacion, material.campos)) {
                materialResult = await materialValidator.validar_check(enums.codigo_interno.ampliacion, material, materialResult, 1, 20);
            }
        } else if (tipo_solicitud == enums.tipo_solicitud.ampliacion) {
        ...
        // [121] motivo
        if (tipo_solicitud == enums.tipo_solicitud.modificacion || tipo_solicitud == enums.tipo_solicitud.bloqueo) {
            if (existeRegla(enums.codigo_interno.motivo, reglas) && existeCampo(enums.codigo_interno.motivo, material.campos)) {
                materialResult = await materialValidator.validar_texto(enums.codigo_interno.motivo, material, materialResult, 3000, 3000);
            }
        }
...
function obtenerCamposVisibles(rep, m, campos, list_clasificacion, list_clase_inspeccion, list_area_planificacion, list_equivalencia, list_anexos, list_nivel_servicio, list_grupo_planif_necesidades, list_modelo_pronostico) {
    ...
    if (campos.some(e => e.codigo_interno === enums.codigo_interno.anular_automaticamente))
        /*[98]*/rep = obtenerRepresentation(enums.codigo_interno.anular_automaticamente, rep, m.anular_automaticamente_borrador, null, m.anular_automaticamente, m.anular_automaticamente_error);
    if (campos.some(e => e.codigo_interno === enums.codigo_interno.optimizacion_parametro))
        /*[99]*/rep = obtenerRepresentation(enums.codigo_interno.optimizacion_parametro, rep, m.optimizacion_parametro_borrador, null, m.optimizacion_parametro, m.optimizacion_parametro_error);
    ...
    if (campos.some(e => e.codigo_interno === enums.codigo_interno.motivo))
        /*[121]*/rep = obtenerRepresentation(enums.codigo_interno.motivo, rep, m.motivo_borrador, null, m.motivo, m.motivo_error);
    ...
    if (campos.some(e => e.codigo_interno === enums.codigo_interno.area_planificacion_tab)) {
        ...
        area_planificacionPorMaterial.forEach(area_planificacion => {
            ids = ids.concat(area_planificacion.area_planificacion, ",")
        ...
        rep[enums.codigo_interno.area_planificacion_tab + "_id"] = ids.substring(0, ids.length - 1);
...
function existeDenominacionExacta(nombresExistentes, denominacion) {
    ...
    if (nombresExistentes.length > 0) {
        ...
        for (let index = 0; index < nombresExistentes.length; index++) {
            const material = nombresExistentes[index];
...
function obtenerCampoValor(codigo_interno, campos) {
    ...
    for (let index = 0; index < campos.length; index++) {
        ...
        if (campo.codigo_interno === codigo_interno) {
            ...
            if (campo.codigo_interno === enums.codigo_interno.ampliacion) {
...
async function clonarPadre(client, id_padre, request_centro, request_almacen) {
    const result = await materialSolicitudService.clonarMaterialSolicitud(client, id_padre, request_centro, request_almacen);
    const id_material_solicitud = result[0].id;
    await materialSolicitudService.clonarMaterialSolicitudBorrador(client, id_padre, id_material_solicitud, request_centro, request_almacen);
    await clasificacionMaterialService.clonar(client, id_padre, id_material_solicitud);
    await claseInspeccionMaterialService.clonar(client, id_padre, id_material_solicitud);
    await areaPlanificacionMaterialService.clonar(client, id_padre, id_material_solicitud);
    await nivelServicioService.clonar(client, id_padre, id_material_solicitud);
    await grupoPlanifNecesidadesMaterialService.clonar(client, id_padre, id_material_solicitud);
    await modeloPronosticoMaterialService.clonar(client, id_padre, id_material_solicitud);

    return id_material_solicitud;
};
