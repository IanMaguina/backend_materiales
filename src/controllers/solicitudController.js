...
solicitudController.aprobacionRechazoSolicitud = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            winston.error("Error notificando la finalización,", error);
        }       
...
solicitudController.finalizarSolicitud = async (req, res) => {
    ...
    try {
        ...
        if (listaMaterialesRes.length > 0) {
            ...
            let list_roles = [];
            for (let index = 0; index < listaAprobadoresRes.length; index++) {
                const element = listaAprobadoresRes[index];
            ...
            for (let i = 0; i < listaAprobadoresRes.length; i++) {
                if (listaAprobadoresRes[i].esta_aqui === true) {
                    data.posicion = i;
                    data.id_rol = listaAprobadoresRes[i].id_rol;
                    data.id_estado_real = listaAprobadoresRes[i].id_estado_real
                    break;
                }
            }
            winston.info("posicion: " + data.posicion);
            if (data.id_rol !== list_roles[list_roles.length - 1]) {
                await client.query("ROLLBACK");
                response.resultado = 0;
            ...
            try {
                await notificacionController.internal.notificarFinalizacion(id_solicitud);
            } catch (error) {
                winston.error("Error notificando la finalización,", error);
            }
