...
sapController.respuestaCreacionSolicitud = async (req, res) => {
    ...
    try {
        ...
        try {
            await notificacionController.internal.notificarRespuestaSAP(id_solicitud, detalle);
        } catch (error) {
            winston.error("Error enviando correos luego de recibir respuesta de SAP,", error);
        }
